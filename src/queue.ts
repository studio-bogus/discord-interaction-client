...
export class Queue {
  ...
  async #process(route: Route, resource: string, init: RequestInit, retries = 0): Promise<unknown> {
    ...
    if (key !== null && key !== undefined && key !== this.id) {
     this.manager.logger.verbose(`Received new bucket hash.`, {
        id: this.id,
        key,
      });
    ...
    if (response.status === 429) {
      ...
      this.manager.logger.verbose(`Encountered 429 rate limit`, {
  ...
  async #rateLimitThrottle() {
    while (this.manager.isGlobalLimited() || this.isLocalLimited()) {
      ...
      if (isGlobal) {
        this.manager.logger.verbose(`Global rate limit reached. Blocking all requests for ${timeout}ms`);
      } else {
        this.manager.logger.verbose(`${this.id} encountered a local rate limit. Waiting ${timeout}ms before continuing`);
      }
